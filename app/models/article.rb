class Article < ApplicationRecord
  ...
  def date_check
    errors.add(:expired_at, 'の日付を正しく記入してください。') unless released_at < expired_at
  end

  scope :public_article, -> do
    now = Time.now
    where('released_at <= ?', now).where('expired_at > ?', now).where(member_only: false)
  end

  scope :member_article, -> do
    now = Time.now
    where('released_at <= ?', now).where('expired_at > ?', now)
  end
end
